...
For use others tools, I found Rustscan which optimizes the scan, by first looking at open ports and then scanning those ports specifically.
...
If we enter the web through the IP, it will redirect us to `artcorp.htb` which we will have to add to the hosts file.
...
Seeing that there are no interesting directories on the domain, we'll see if it has subdomains again with [ffuf](https://github.com/ffuf/ffuf).
...
This is the web of the subdomain. You have a link to an application under development.

![dev01](dev01.png)
_Subdomain website_

When entering the metaview directory we find an image metadata reader. If we introduce any image we see that it returns the following:
...
We will use `searchsploit` to see if there is an exploit for exiftool. We see that yes.
...
The first thing is to see how the exploit works and inform us about it in [CVE-2021-22204](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2021-22204).
...
> This exploit need djvulibre-bin and exiftool packages
{: .prompt-info}


In the help we can see that using the `-s` option generates a payload with a reverse shell.
...
In the netcat that we would have previously left listening, we see how it has worked and we will be inside as the user `www-data`.
...
After doing a basic enumeration and finding nothing. I decided to use pspy64 to see what processes are running in the background and who is running them.

We found running a script with imagemagick in it and mogrify. 
...
Searching for ways to exploit that run that seemed to be in crontab, I found the following:
...
We downloaded the image by mounting a web server with python on our host and using wget on the victim machine pointing to the file.
...
I was lucky on the climb because the first thing I usually do is `sudo -l` and it almost never gives me anything interesting but this time it didn't.
...
It is a machine that to be medium difficulty is "easy". I guess the difficulty is higher for the downstream user (www-data). I had not dealt with images for this type of thing and I find it interesting.
